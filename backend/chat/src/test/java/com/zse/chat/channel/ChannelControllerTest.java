...
class ChannelControllerTest {
    ...
    for (int i = 0; i < 5; i++) {
      channels.add(
          ChannelFixture.createDefaultChannel(
              ...
              UserFixture.createSetOfDefaultUser(2, 1),
              null
          ).build()
      );
    }
  ...
  public void shouldValidateCreateChannelResponse() throws Exception {
    ...
    mockMvc.perform(post("/channels").headers(authorize()))
        ...
        .andExpect(jsonPath("$.owners", hasItem("testNickname1")))
        .andExpect(jsonPath("$.members", hasSize(1)))
        .andExpect(jsonPath("$.members", hasItem("testNickname2")));
  ...
  public void shouldAddChannelOwner() throws Exception {
    ...
    final Set<User> ownersBefore = UserFixture.createSetOfDefaultUser(1);
    final Set<User> ownersAfter = UserFixture.createSetOfDefaultUser(2);
    final Set<User> membersBefore = UserFixture.createSetOfDefaultUser(2, 1);
    final Set<User> membersAfter = new HashSet<>();
    ...
    mockMvc.perform(put("/channels/users").headers(authorize())
        ...
        .andExpect(jsonPath("$.owners", hasItem("testNickname1")))
        .andExpect(jsonPath("$.owners", hasItem("testNickname2")))
  ...
  public void shouldRemoveChannelOwner() throws Exception {
    ...
    final Set<User> ownersBefore = UserFixture.createSetOfDefaultUser(2);
    final Set<User> ownersAfter = UserFixture.createSetOfDefaultUser(1);
    final Set<User> membersBefore = new HashSet<>();
    final Set<User> membersAfter = UserFixture.createSetOfDefaultUser(2, 1);
  ...
  public void shouldAddChannelMember() throws Exception {
    ...
    final Set<User> owners = UserFixture.createSetOfDefaultUser(1);
    final Set<User> membersBefore = new HashSet<>();
    final Set<User> membersAfter = UserFixture.createSetOfDefaultUser(2, 1);
